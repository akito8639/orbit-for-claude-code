...
- **What it reads**: the OAuth token Claude Code stores in your keychain (or `~/.claude/.credentials.json`), Claude Code's session files under `~/.claude`, the tail of session transcripts for context-window numbers, today's transcript files for token totals, the working-folder paths in transcripts and `~/.claude.json` (repository names for the Sessions filter), and the Claude desktop app's local Cowork session records.
